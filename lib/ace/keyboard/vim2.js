...
define(function(require, exports, module) {
  ...
  CodeMirror.lookupKey = function(name, maps, handle) {
    function lookup(map) {
      if (typeof map == "string")
        map = CodeMirror.keyMap[map];
      var found = typeof map == "function" ? map(name) : map[name];
  ...
  CodeMirror.prototype.destroy = function() {
    Object.keys(this.marks).forEach(function(x) {
      this.marks[x].clear();
    }, this);
  ...
  CodeMirror.prototype.onChange = function(delta) {
    if (delta.action[0] == 'i') {
  ...
  CodeMirror.prototype.onSelectionChange = function() {
    var curOp = this.curOp = this.curOp || {};
  ...
  CodeMirror.prototype.operation = function(fn, force) {
  ...
  CodeMirror.prototype.onBeforeEndOperation = function() {
    var op = this.curOp;
    if (op) {
      this.ace.session.markUndoGroup();
  ...
  CodeMirror.prototype.signal = function(eventName, e, handlers) {
  ...
  CodeMirror.prototype.firstLine = function() { return 0; };
  CodeMirror.prototype.lastLine = function() { return this.ace.session.getLength() - 1; };
  CodeMirror.prototype.lineCount = function() { return this.ace.session.getLength(); };
  ...
  CodeMirror.prototype.getCursor = function(p) {
    var sel = this.ace.selection;
    var pos = p == 'anchor' ? (sel.isEmpty() ? sel.lead : sel.anchor) :
        p == 'head' || !p ? sel.lead : sel.getRange()[p];
    return toCmPos(pos);
  };
  CodeMirror.prototype.listSelections = function(p) {
  ...
  CodeMirror.prototype.setSelections = function(p, primIndex) {
  ...
  CodeMirror.prototype.setSelection = function(a, h, options) {
    var sel = this.ace.selection;
  ...
  CodeMirror.prototype.somethingSelected = function(p) {
    return !this.ace.selection.isEmpty();
  };
  CodeMirror.prototype.clipPos = function(p) {
    var pos = this.ace.session.$clipPositionToDocument(p.line, p.ch);
    return toCmPos(pos);
  };
  CodeMirror.prototype.markText = function(cursor) {
    // only used for fat-cursor, not needed
    return {clear: function() {}, find: function() {}};
  };
  CodeMirror.prototype.$updateMarkers = function(delta) {
  ...
  CodeMirror.prototype.setBookmark = function(cursor, options) {
  ...
  CodeMirror.prototype.moveH = function(increment, unit) {
    if (unit == 'char') {
      var sel = this.ace.selection;
      sel.clearSelection();
  ...
  CodeMirror.prototype.findPosV = function(start, amaount, unit, goalColumn) {
    if (unit == 'line') {
  ...
  CodeMirror.prototype.getSearchCursor = function(query, pos, caseFold) {
  ...
  CodeMirror.prototype.scrollTo = function(x, y) {
    var renderer = this.ace.renderer;
  ...
  CodeMirror.prototype.scrollInfo = function() { return 0; };
  CodeMirror.prototype.scrollIntoView = function() { return 0; };
  CodeMirror.prototype.getLine = function(row) { return this.ace.session.getLine(row) };
  CodeMirror.prototype.getRange = function(s, e) {
    return this.ace.session.getTextRange(new Range(s.line, s.ch, e.line, e.ch));
  };
  CodeMirror.prototype.replaceRange = function(text, s, e) {
    if (!e) e = s;
    return this.ace.session.replace(new Range(s.line, s.ch, e.line, e.ch), text);
  };
  CodeMirror.prototype.replaceSelections = function(p) {
    var sel = this.ace.multiSelect;
  ...
  CodeMirror.prototype.getSelection = function() {
    return this.ace.getSelectedText();
  };
  CodeMirror.prototype.getSelections = function() {
  ...
  CodeMirror.prototype.setOption = function(name, val) {
    this.state[name] = val;
    switch (name) {
      case 'indentWithTabs':
        name = optMap[name];
  ...
  CodeMirror.prototype.getOption = function(name, val) {
  ...
  CodeMirror.prototype.toggleOverwrite = function(on) {
    this.state.overwrite = on;
    return this.ace.setOverwrite(on);
  };
  CodeMirror.prototype.addOverlay = function(re) {
    return this.ace.session.highlight(re);
  };
  CodeMirror.prototype.removeOverlay = function(re) {
    return this.ace.session.highlight(null);
  };
  CodeMirror.prototype.getScrollInfo = function() {
    var renderer = this.ace.renderer;
    var config = renderer.layerConfig;
    return {
  ...
  CodeMirror.prototype.setValue = function(v) {
    return this.ace.setValue(v);
  };
  CodeMirror.prototype.getTokenTypeAt = function(pos) {
    var token = this.ace.session.getTokenAt(pos.line, pos.ch);
    return token && /comment|string/.test(token.type) ? "string" : "";
  };
  CodeMirror.prototype.findMatchingBracket = function(pos) {
    var m = this.ace.session.findMatchingBracket(toAcePos(pos));
    return {to: m && toCmPos(m)};
  };
  CodeMirror.prototype.indentLine = function(line, method) {
    if (method === true)
        this.ace.session.indentRows(line, line, "\t");
    else if (method === false)
        this.ace.session.outdentRows(new Range(line, 0, line, 0));
  };
  CodeMirror.prototype.indexFromPos = function(pos) {
    return this.ace.session.doc.positionToIndex(toAcePos(pos));
  };
  CodeMirror.prototype.posFromIndex = function(index) {
    return toCmPos(this.ace.session.doc.indexToPosition(index));
  };
  CodeMirror.prototype.focus = function(index) {
    return this.ace.focus();
  };
  CodeMirror.prototype.blur = function(index) {
    return this.ace.blur();
  };
  CodeMirror.prototype.defaultTextHeight = function(index) {
    return this.ace.renderer.layerConfig.lineHeight;
  };
  CodeMirror.prototype.scanForBracket = function(pos, dir, _, options) {
...
dom.importCssString(".ace_dialog {\
  position: absolute;\
  left: 0; right: 0;\
  background: white;\
  z-index: 15;\
...
(function() {
  ...
  var Vim = function() {
    ...
    var actions = {
      ...
      fold: function(cm, actionArgs, vim) {
  ...
  exports.handler = {
    ...
    attach: function(editor) {
      ...
      var vim = CodeMirror.Vim.maybeInitVimState_(cm);
      CodeMirror.vimModeOption(cm, true);
      vim.status = null;
    ...
    getStatusText: function() {
        var vim = CodeMirror.Vim.maybeInitVimState_(this.cm);
        if (vim.insertMode)
            return "INSERT";
        var status = "";
        if (vim.visualMode) {
